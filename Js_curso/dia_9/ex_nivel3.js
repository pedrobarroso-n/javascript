...
console.log(countries.map(mostre => mostre.name).sort())
...
function mostSpeakLanguages(countries, num){
  ...
  let count = 0
  for (let c = 0; c < allLanguages.length; c++){
    for (let i = allLanguages.length-1; i >= 0; i--){
      allLanguages[i] === allLanguages[c] ? count++ : count+=0
    }
    mostSpeak.push({language: allLanguages[c], counts: count})
    allLanguages = allLanguages.filter(remove => remove != allLanguages[c]) // excluir as ja usadas
    count = 0
  }

  /***ordenar por mais repetições***/
  mostSpeak.sort((a, b) => b.counts - a.counts)
